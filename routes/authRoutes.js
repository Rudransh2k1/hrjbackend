...
const { 
    ...
    getAllMdForAdmin,
    getAllDForAdmin,
    getAllRForAdmin } = require('../models/dbOperations');

// Route for user registration
router.post('/register', async (req, res) => {
    try {
        const { name, email, password, user_Type, category, middleName, lastName, mobileNumber, outletName, aadharcardNumber, gstin, dateOfBirth, bankAccountNumber, ifsc, address, pincode, district, state, city, alternateNumber, pancardNumber } = req.body;
        ...
        // Create new user
        await createUser(name, email, password, user_Type, parentId, category, middleName, lastName, mobileNumber, outletName, aadharcardNumber, gstin, dateOfBirth, bankAccountNumber, ifsc, address, pincode, district, state, city, alternateNumber, pancardNumber);
...
router.get('/adminusers', async (req, res) => {
    try {
        ...
        if (user.user_type !== 'Admin') {
            return res.status(403).json({ message: 'Unauthorized access. Only Admin users are permitted to use this functionality.' });
        }
